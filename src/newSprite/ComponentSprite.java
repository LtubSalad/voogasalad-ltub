...
/**
 * 
 * @author Daniel A new version of the Sprite (still in progress). Stores a
 *         bunch of components, (which are similar to attributes but for their
 *         ability to handle and fire events). Every ComponentSprite also
 *         handles events concerning instantiation and removal. Note the nested
 *         classes. The ComponentSprite stores nested classes for events, but
 *         also the abstract Component class. Hence every component has access
 *         to the
 *
 */
public abstract class ComponentSprite {
	...
	protected BasicEventBus environmentBus;
	private ScriptEngine myEngine;
	private Collection<Component<? extends BusEvent>> myComponents;// Use
																	// keping's
																	// method
	...
	/**
	 * 
	 * @author Daniel Setting position, tracking position, and image?
	 * @param <T>
	 */

	public abstract class Component<T extends BusEvent> {
		protected BasicEventBus bus;
		protected BasicEventBus eBus;
		...
		public abstract Collection<BusEventType<T>> getListenedEvents();
	}
}
